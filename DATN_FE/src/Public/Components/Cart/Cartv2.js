...
function Cart() {
    ...
    const handleQuantityChange = async (id, qty) => {
        try {
            ...
            const currentCart = carts.find(item => item._id === id);
            if (!currentCart) return;

            if (qty > currentCart.product_id.quantity) {
                message.warning(`Chỉ còn ${currentCart.product_id.quantity} sản phẩm trong kho`);
                return;
            }
                ...
                prevCarts.map(cart =>
                    cart._id === id
                        ? {
                            ...
                            total: (cart.product_id.sale_price || cart.product_id.price) * updatedCart.quantity
                        }
                        : cart
                )
            );

            const price = currentCart.product_id.sale_price || currentCart.product_id.price;
            const total = price * updatedCart.quantity;
            $(`#totalCartItem${id}`).text(ConvertCurrency(total));
            calcTotal();
        } catch (err) {
    ...
    const plusQuantity = async (el) => {
        ...
        const cart_id = input.data('id');
        const currentCart = carts.find(item => item._id === cart_id);
    ...
    const calcTotal = () => {
        ...
        $('.totalCartItem').each(function() {
            let itemTotal = $(this).text().replaceAll(',', '').replaceAll('.', '').replaceAll('đ', '');
                                                            ...
                                                            >
                                                                <div className="cart-item-image">
                                                                    <img
                                                                        ...
                                                                        alt={cart.product_id.title}
                                                                        className="img-fluid rounded"
                                                                    />
                                                                </div>
                                                                <div className="cart-item-content">
                                                                    <div className="cart-item-details">
                                                                        <h5 className="cart-item-title">{cart.product_id.title}</h5>
                                                                        <div className="cart-item-price">
                                                                            {ConvertCurrency(cart.product_id.sale_price || cart.product_id.price)}
                                                                        </div>
                                                                    </div>
                                                                    <div className="cart-item-actions">
                                                                        <div className="quantity-control">
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                data-id={cart._id}
                                                                                data-price={cart.product_id.sale_price || cart.product_id.price}
                                                                            />
                                                                            <button
                                                                                className="btn btn-quantity"
                                                                        ...
                                                                        <div className="cart-item-total">
                                                                            <span className="totalCartItem" id={`totalCartItem${cart._id}`}>
                                                                                {ConvertCurrency((cart.product_id.sale_price || cart.product_id.price) * cart.quantity)}
                                                                            </span>
                                                                        </div>
                                                                        <button
